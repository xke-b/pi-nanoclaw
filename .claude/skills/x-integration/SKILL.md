...
npx dotenv -e .env -- npx tsx .pi/skills/x-integration/scripts/setup.ts
# Verify: data/x-auth.json should exist after successful login

# 2. Rebuild container to include skill
./container/build.sh
# Verify: Output shows "COPY .pi/skills/x-integration/agent.ts"
...
```typescript
import { handleXIpc } from '../.pi/skills/x-integration/host.js';
```
...
```typescript
// @ts-ignore - Copied during Docker build from .pi/skills/x-integration/
...
Change build context from `container/` to project root (required to access `.pi/skills/`):
```bash
# Find:
...
First, update the build context paths (required to access `.pi/skills/` from project root):
```dockerfile
# Find:
...
COPY .pi/skills/x-integration/agent.ts ./src/skills/x-integration/
```
...
```bash
echo '{"content":"Test tweet - please ignore"}' | npx dotenv -e .env -- npx tsx .pi/skills/x-integration/scripts/post.ts
```

### Test Like

```bash
echo '{"tweetUrl":"https://x.com/user/status/123"}' | npx dotenv -e .env -- npx tsx .pi/skills/x-integration/scripts/like.ts
```
...
echo '{"content":"Test"}' | npx tsx .pi/skills/x-integration/scripts/post.ts
```
