...
## 4. Pi Authentication (No Script)

If HAS_ENV=true from step 2, read `.env` and check for Pi provider keys (for example `ANTHROPIC_API_KEY`). If present, confirm with user: keep or reconfigure?

AskUserQuestion: Pi interactive login (`/login`) or API key env vars?

**Pi login:** Tell user to run `pi` then `/login` in another terminal, and do not paste secrets in chat.
...
**Container agent fails ("Pi Coding Agent process exited with code 1"):** Ensure the container runtime is running — `open -a Docker` (macOS Docker), `container system start` (Apple Container), or `sudo systemctl start docker` (Linux). Check container logs in `groups/main/logs/container-*.log`.
