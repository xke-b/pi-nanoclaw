...
This skill adds a stdio-based MCP server that exposes local Ollama models as tools for the container agent. Pi remains the orchestrator but can offload work to local models.
...
```bash
npx tsx scripts/apply-skill.ts .pi/skills/add-ollama-tool
```
