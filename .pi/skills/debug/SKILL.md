...
src/container-runner.ts               container/agent-runner/
    │                                      │
    │ spawns container                      │ runs Pi SDK
    │ with volume mounts                   │ with MCP servers
    │                                      │
    ├── data/env/env ──────────────> /workspace/env-dir/env
    ├── groups/{folder} ───────────> /workspace/group
    ├── data/ipc/{folder} ────────> /workspace/ipc
    ├── data/sessions/{folder}/.pi-agent/ ──> /home/node/.pi/agent/ (isolated per-group)
    └── (main only) project root ──> /workspace/project
```

**Important:** The container runs as user `node` with `HOME=/home/node`. Session files must be mounted to `/home/node/.pi/agent/` (not `/root/.pi/agent/`) for session resumption to work.
...
| **Pi sessions** | `~/.pi/agent/sessions/` | Pi session history |
...
### 1. "Pi Coding Agent process exited with code 1"
...
# # Use Pi /login or provider API keys  (subscription)
...
**Workaround:** The system extracts only authentication variables (`ANTHROPIC_API_KEY` and other provider keys) from `.env` and mounts them for sourcing inside the container. Other env vars are not exposed.
...
```
/workspace/
├── env-dir/env           # Environment file (provider API keys)
...
### 5. Session Not Resuming / "Pi Coding Agent process exited with code 1"

If sessions aren't being resumed (new session ID every time), or Pi Coding Agent exits with code 1 when resuming:

**Root cause:** The runtime uses `$HOME/.pi/agent/sessions/`. Inside the container, `HOME=/home/node`, so it looks at `/home/node/.pi/agent/projects/`.

**Check the mount path:**
```bash
# In container-runner.ts, verify mount is to /home/node/.pi/agent/, NOT /root/.pi/agent/
grep -A3 "pi" src/container-runner.ts
```
...
echo "HOME=$HOME"
ls -la $HOME/.pi/agent/sessions/ 2>&1 | head -5
'
```

**Fix:** Ensure `container-runner.ts` mounts to `/home/node/.pi/agent/`:
```typescript
mounts.push({
  hostPath: claudeDir,
  containerPath: '/home/node/.pi/agent',  // NOT /root/.pi/agent
  readonly: false
});
```
...
The agent-runner uses these Pi SDK options:
...
**Important:** `allowDangerouslySkipPermissions: true` is required when using `permissionMode: 'bypassPermissions'`. Without it, Pi Coding Agent exits with code 1.
...
Pi sessions are stored per-group in `data/sessions/{group}/.pi-agent/` for security isolation. Each group has its own session directory, preventing cross-group access to conversation history.

**Critical:** The mount path must match the container user's HOME directory:
- Container user: `node`
- Container HOME: `/home/node`
- Mount target: `/home/node/.pi/agent/` (NOT `/root/.pi/agent/`)
...
rm -rf data/sessions/{groupFolder}/.pi-agent/
...
[ -f .env ] && grep -Eq "(ANTHROPIC_API_KEY|OPENAI_API_KEY|GEMINI_API_KEY|ZAI_API_KEY)=" .env && echo "OK" || echo "MISSING - add provider API keys to .env"
...
grep -q "/home/node/.pi/agent" src/container-runner.ts 2>/dev/null && echo "OK" || echo "WRONG - should mount to /home/node/.pi/agent/, not /root/.pi/agent/"
