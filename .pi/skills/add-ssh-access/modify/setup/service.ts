...
function setupNohupFallback(
  ...
  const lines = [
    ...
    '',
    '# Persist and reuse SSH agent across restarts so SSH forwarding remains available.',
    'AGENT_ENV_DIR="$HOME/.cache/nanoclaw"',
    'AGENT_ENV_FILE="$AGENT_ENV_DIR/ssh-agent.env"',
    ...
    '  if ssh-add -l >/tmp/nanoclaw-sshadd.$$ 2>&1; then',
    '    SSH_READY=true',
    '  elif grep -q "The agent has no identities" /tmp/nanoclaw-sshadd.$$; then',
    '    SSH_READY=true',
    '  fi',
    'fi',
    '',
